...
#AUDIO: dice.mp3
    ~ typical(jonesFace)
{currentRoll:
    <stat>Face<end> - {chance(jonesFace)} <inc>Success
    ...
    - else:
    <stat>Face<end> - {chance(jonesFace)} <dec>Failure
...
Well, this <i>is</i> a paranormal class, so you suppose it makes sense. {jsos > 1:Even if the professor seems weirder than necessary.}
...
Brigid glaces between the three of you. "I-it's nothing. Uh, Jones? Did you have something?"
    ...
    * [Make something up to cover for Brigid.]
        ...
        {jonesKnows ? syllabus:
            ~alter(jos, 1)
              <rel>⇑ ⇑
            "Oh. Yeah. Totally," you say, brain racing. "I, uh, was just wondering about some of the topics on the syllabus? They weren't what I was expecting from a paranormal class."
            - else:
              <rel>⇑
...
There's a sort of silent expectation when you're all loaded in the car and on the road, and you realize they're waiting on you to explain with no further prompting.
    ...
    * {(jbrig > 1) && (jfel > 1)}[Tell them everything, even the weird stuff.]
        -> tell_weird
...
You take a deep breath and get out of the car. Then Felixe shuts off the engine and does the same; Brigid is already out and waiting on the sidewalk as soon as the car quiets. They both follow you to the entrance where you pause and look back at them. "You know you can't, like, come in with me, right?"
   * [➔]
   ...
   #CLEAR
   ~alter(brigf,-1)
   Felixe raises his eyebrows. "They've blocked off the whole mall for you to sign paperwork? {felixeKnows == felixeKnows.dadCult: Maybe this <i>is</i> a cult thing.}" His tone is so flat you almost don't catch the sarcasm.
...
You take a deep breath in, then let it out, and try not to flinch as the pen slices into your fingertip.
    ...
    {(diceRoll > 7) || (diceRoll2 > 7):
        {diceRoll > 6:
        <stat>Muscle<end> - {chance(jonesMuscle)} <inc>Success
        - else:
        <stat>Fixer<end> - {chance(jonesFixer)} <inc>Success
        }
        It didn't hurt as bad as you thought it would. You've had worse papercuts.
    - else:
        
    {diceRoll > diceRoll2:
        <stat>Muscle<end> - {chance(jonesMuscle)} <dec>Failure
        -else:
        <stat>Muscle<end> - {chance(jonesMuscle)} <dec>Failure
        }
...
When you remove your finger, all you see is a deep red blotch bleeding into the lines of the parchment. You're not sure what you expected -- it to magically turn into your name or something? -- but you almost laugh at how mundane it is.
    ...
    *[➔]
-
     ~temp askedAboutPapers = false
    The folder is titled <task>Project Caedes<end>, which doesn't mean anything to you, and the files inside mostly have headings you'd expect.
...
*[➔]
    -
He smiles. "Don't play coy, Private Jones. The two who arrived with you today. Brigid Flint and Felixe..." He pauses and glances at one of the papers on his desk. For a split second, you think you see his brow furrow. "They're the ones waiting for you, correct?" he finishes, as though the pause was intentional.
"How did you --"
...
You find Brigid and Felixe loitering outside the Hot Topic, and you can't tell if it's funnier if they've been out here the whole time, or if you've found them after an unproductive shopping trip inside. Either way, you laugh as you approach, which gets both of their attention.
    ...
    {felj > 0:
        ...
        {brigidKnows has brigidKnows.dadCult:
            ...
            - else:
            "I'm sure it wasn't as bad as all that, right?" Brigid asks, clearly worried.
        }
    }
    * {jsbrig == 0 && jsfel == 0}[Tell them about signing in blood.]
        ~felixeKnows += felixeKnows.signBlood
        ~brigidKnows += brigidKnows.signBlood
        {felj <= 0:
            "About that..." you begin.
            Felixe's eyebrows shoot up.
            }
        "So, actually, they made me sign in blood?"
        "What?" Felixe and Brigid ask at once, their respective and contrasting deadpan and frantic tones almost making you want to laugh.
...
{jfel < 0:You roll your eyes, but otherwise ignore him.|"Yeah, I guess we're kind of past that point anyway," you say with a laugh.} "What I mean is, it's probably a coincidence, but you guys read the syllabus right?"
...
Felixe shrugs{jfel > 1: a little sheepishly}. "I just think it's too early to be jumping to conclusions."
...
= osbourne
 ...
 "That's one way to put it," you say with a laugh. <> {jsos > 0:
